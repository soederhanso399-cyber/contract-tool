...
function parseBasicInfo(text) {
  const result = {};
  const patterns = [
    { keys:["公司名称","乙方名称","单位名称","企业名称","名称","公司"],  field:"party_b_name"   },
    { keys:["统一社会信用代码","信用代码","税号","社会信用代码","纳税人识别号"], field:"party_b_credit" },
    { keys:["地址","公司地址","注册地址","联系地址"],                    field:"party_b_address"},
    { keys:["联系人","经办人","负责人","姓名"],                          field:"party_b_contact"},
    { keys:["电话","手机","联系电话","手机号","联系方式"],               field:"party_b_phone"  },
  ];
  const normalized = text.replace(/[：:]\s*/g,"：").replace(/\r/g,"");
  patterns.forEach(({keys,field})=>{
    for(const key of keys){
      const re = new RegExp(`${key}[：:]([^\n，,；;　]+)`,"i");
      const m = normalized.match(re);
      if(m&&m[1].trim()){result[field]=m[1].trim();break;}
    }
  });
  // 如果没识别到名称但第一行像公司名（含"公司"/"有限"），直接用
  if(!result.party_b_name){
    const firstLine = text.trim().split("\n")[0].trim();
    if(firstLine&&(firstLine.includes("公司")||firstLine.includes("有限")||firstLine.includes("企业"))){
      result.party_b_name = firstLine;
    }
  }
  return result;
}
...
export default function App() {
  ...
  // 基本信息粘贴
  const [basicPaste, setBasicPaste]     = useState("");
  const [basicOpen, setBasicOpen]       = useState(false);
  const [basicPreview, setBasicPreview] = useState(null);
  ...
  // 基本信息解析
  const doBasicParse = () => {
    if(!basicPaste.trim()){setBasicErr("请粘贴基本信息文本");return;}
    const r=parseBasicInfo(basicPaste);
    if(!Object.keys(r).length){setBasicErr("未能识别字段，请确认包含公司名称/地址/联系人等关键词");return;}
    setBasicPreview(r); setBasicErr("");
  };
  const doBasicApply = () => {
    if(!basicPreview) return;
    Object.entries(basicPreview).forEach(([id,v])=>setField(id,v));
    setBasicPreview(null); setBasicPaste(""); setBasicOpen(false);
    notify("基本信息已自动填充");
  };
  ...
  const BASIC_LABELS = {
    party_b_name:"乙方名称", party_b_credit:"统一社会信用代码",
    party_b_address:"乙方地址", party_b_contact:"乙方联系人", party_b_phone:"乙方电话",
  };
            ...
            {activeSec!=="产品清单"?(
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
                  ...
                  {/* 基本信息/结算信息：显示粘贴按钮 */}
                  {activeSec==="基本信息"&&(
                    <BtnX label={basicOpen?"▲ 收起":"📋 粘贴文本自动填充"} onClick={()=>{setBasicOpen(!basicOpen);setBasicPreview(null);}} variant={basicOpen?"outline":"ghost"}/>
                  )}
                  {activeSec==="结算信息"&&(
                ...
                {/* 基本信息粘贴区 */}
                {activeSec==="基本信息"&&basicOpen&&(
                  <div style={{marginBottom:24,animation:"fadeIn .2s ease"}}>
                    <div style={{padding:"16px 18px",borderRadius:10,border:`1.5px solid ${basicPreview?"#059669":T.border}`,background:basicPreview?"rgba(5,150,105,0.04)":T.sidebar,boxShadow:T.shadow}}>
                      <div style={{fontSize:12,color:T.textMd,marginBottom:9,fontFamily:"system-ui",fontWeight:600}}>
                        📋 粘贴乙方基本信息，自动识别公司名称、信用代码、地址、联系人、电话
                      </div>
                      <div style={{fontSize:11,color:T.textMuted,marginBottom:10,fontFamily:"system-ui",lineHeight:1.7}}>
                        支持格式举例：<br/>
                        <code style={{background:T.high,padding:"2px 6px",borderRadius:4,fontSize:11}}>公司名称：XX科技有限公司  统一社会信用代码：91XXXXXX  地址：XX省XX市XX路  联系人：张三  电话：138XXXX</code>
                      </div>
                      <textarea
                        value={basicPaste}
                        onChange={e=>{setBasicPaste(e.target.value);setBasicPreview(null);setBasicErr("");}}
                        placeholder={"公司名称：南京某某科技有限公司\n统一社会信用代码：91320115XXXXXXXX\n地址：江苏省南京市江宁区XX路XX号\n联系人：张三\n电话：13800138000"}
                        rows={4}
                        style={{width:"100%",borderRadius:8,padding:"10px 12px",border:`1.5px solid ${basicErr?"#dc2626":T.border}`,background:"#fff",color:T.text,fontSize:12,fontFamily:"monospace",resize:"vertical",outline:"none",boxSizing:"border-box",lineHeight:1.7,boxShadow:"inset 0 1px 2px rgba(0,0,0,0.04)"}}
                      />
                      {basicErr&&<div style={{fontSize:11,color:T.err,marginTop:5,fontFamily:"system-ui"}}>⚠ {basicErr}</div>}
                      {basicPreview&&(
                        <div style={{marginTop:12,padding:"12px 14px",borderRadius:8,background:T.okBg,border:`1px solid ${T.ok}40`}}>
                          ...
                          {Object.entries(basicPreview).map(([id,v])=>(
                            <div key={id} style={{display:"flex",gap:8,marginBottom:4,fontFamily:"system-ui",fontSize:12}}>
                              <span style={{color:T.textMuted,minWidth:110}}>{BASIC_LABELS[id]||id}：</span>
                      ...
                      <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
                        <BtnX label="🔍 解析识别" onClick={doBasicParse} variant="primary"/>
                        {basicPreview&&<BtnX label="✅ 确认填入" onClick={doBasicApply} variant="success"/>}
                        {basicPaste&&<BtnX label="清空" onClick={()=>{setBasicPaste("");setBasicPreview(null);setBasicErr("");}} variant="ghost"/>}
                      </div>
                    </div>
                  </div>
                )}
